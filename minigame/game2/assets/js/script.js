// 队列类 - 用于迷宫生成
class Queue {
    constructor() {
        this.queue = []
    }
    push(pos) {
        if (Math.random() < 0.5) {
            this.queue.push(pos)
        } else {
            this.queue.unshift(pos)
        }
    }
    pop() {
        if (Math.random() < 0.5) {
            return this.queue.pop()
        } else {
            return this.queue.shift()
        }
    }
    empty() {
        return !this.queue.length
    }
}

// 栈类 - 用于迷宫求解
class Stack {
    constructor() {
        this.stack = []
    }
    push(pos) {
        this.stack.push(pos)
    }
    pop() {
        return this.stack.pop()
    }
    empty() {
        return !this.stack.length
    }
}

// 迷宫类
class Maze {
    constructor(row, col, width = 500, height = 500) {
        // 类名
        this.class = Math.random().toFixed(2)
        // Maze行列
        this.row = row
        this.col = col
        // 迷宫的长宽
        this.width = width
        this.height = height
        // 设置路与墙
        this.road = ' '
        this.wall = '#'
        // 入口坐标（1, 0）
        this.entryX = 1
        this.entryY = 0
        // 出口坐标（倒数第二行， 最后一列）
        this.outX = row - 2
        this.outY = col - 1
        // 迷宫数据
        this.maze = []
        // 生成迷宫时各节点的遍历情况
        this.visited = []
        // 求解迷宫时各节点的遍历情况
        this.findPathVisited = []
        // 设置上下左右的偏移坐标值（上右下左）
        this.offset = [[-1, 0], [0, 1], [1, 0], [0, -1]]
        // 迷宫是否生成完成
        this.hasDown = false
        // 迷宫是否有解
        this.hasFindPath = false
        // 存储迷宫某点的上一点位置
        this.path = []
    }

    // 初始化迷宫数据
    initData(maze) {
        for (let i = 0; i < this.row; i++) {
            maze[i] = new Array(this.col).fill(this.wall) //  初始化二维数组
            this.visited[i] = new Array(this.col).fill(false) // 初始化访问状态为false
            this.findPathVisited[i] = new Array(this.col).fill(false) // 初始化访问状态为false
            this.path[i] = new Array(this.col).fill(null) // 初始化所有元素的上一个元素为null
            for (let j = 0; j < this.col; j++) {
                // 横纵坐标均为奇数 则是路
                if (i % 2 === 1 && j % 2 === 1) {
                    maze[i][j] = this.road
                }
            }
        }
        // 入口及出口 则是路
        maze[this.entryX][this.entryY] = this.road
        maze[this.outX][this.outY] = this.road

        return maze
    }
    
    // 初始化迷宫DOM
    initDOM(maze) {
        let oDiv = document.createElement('div')
        oDiv.style.width = this.width + 'px'
        oDiv.style.height = this.height + 'px'
        oDiv.style.display = 'flex'
        oDiv.className = 'maze-box'
        oDiv.style.flexWrap = 'wrap'
        for (let i = 0; i < maze.length; i++) {
            for (let j = 0; j < maze[i].length; j++) {
                let oSpan = document.createElement('div')
                oSpan.dataset.index = i + '-' + j + '-' + this.class
                oSpan.style.width = (this.width / this.col).toFixed(2) + 'px'
                oSpan.style.height = (this.height / this.row).toFixed(2) + 'px'
                oSpan.style.background = maze[i][j] === this.wall ? '#34495e' : '#ecf0f1'
                oSpan.className = 'cell'
                
                if (i === this.entryX && j === this.entryY) {
                    oSpan.classList.add('entry');
                } else if (i === this.outX && j === this.outY) {
                    oSpan.classList.add('exit');
                }
                
                oDiv.appendChild(oSpan)
            }
        }
        
        // 删除旧的迷宫
        let oldMaze = document.querySelector('.maze-box');
        if (oldMaze) {
            document.getElementById('maze').removeChild(oldMaze);
        }
        
        document.getElementById('maze').appendChild(oDiv)
    }

    // 初始化迷宫
    initMaze() {
        // 迷宫数据
        let maze = this.initData(this.maze)
        // 初始迷宫DOM
        this.initDOM(maze)
    }

    // 重新渲染迷宫 改变的格子坐标为（i, j）
    resetMaze(x, y, type) {
        // 只有不越界的点才做后续处理
        if (this.isArea(x, y)) {
            // 改变this.maze中对应的数据
            this.maze[x][y] = type
            // 改变dom中对应的节点颜色
            let changeSpan = document.querySelector(`div[data-index='${x}-${y}-${this.class}']`)
            changeSpan.style.background = type === this.wall ? '#34495e' : '#ecf0f1'
        }
    }

    // 渲染迷宫
    paintMaze() {
        this.initMaze()

        let queue = new Queue()
        queue.push({ x: this.entryX, y: this.entryY + 1 })
        this.visited[this.entryX][this.entryY + 1] = true

        while (!queue.empty()) {
            let curPos = queue.pop()
            for (let i = 0; i < 4; i++) {
                let newX = curPos.x + this.offset[i][0] * 2  // 两步是 *2
                let newY = curPos.y + this.offset[i][1] * 2
                // 坐标没有越界 而且 没有被访问过
                if (this.isArea(newX, newY) && !this.visited[newX][newY]) {
                    this.resetMaze((newX + curPos.x) / 2, (newY + curPos.y) / 2, this.road) // 打通两个方块之间的墙
                    queue.push({ x: newX, y: newY })
                    this.visited[newX][newY] = true
                }
            }
        }
        this.hasDown = true
        return this
    }

    // 判断坐标是否越界
    isArea(x, y) {
        return x > 0 && x < this.row - 1 && y > 0 && y < this.col - 1 || x == this.entryX && y == this.entryY || x == this.outX && y == this.outY
    }

    // 迷宫自动求解
    findPath(iss) {
        if (!this.hasDown) return alert('请等待迷宫生成后再求解！')
        
        // 重置求解状态
        for (let i = 0; i < this.row; i++) {
            this.findPathVisited[i] = new Array(this.col).fill(false);
            this.path[i] = new Array(this.col).fill(null);
        }
        
        let stack = new Stack()
        stack.push({ x: this.entryX, y: this.entryY }) // 开始
        
        while (!stack.empty()) {
            let curPos = stack.pop()
            this.findPathVisited[curPos.x][curPos.y] = true // 求解时访问过
            
            // 找到出口
            if (curPos.x === this.outX && curPos.y === this.outY) {
                this.hasFindPath = true

                // 绘制解
                let prePos = this.path[curPos.x][curPos.y] // 获取上一个点
                while (prePos != null) {
                    this.findPathReset(prePos.x, prePos.y, '#ff5252')  // 渲染上一个点
                    prePos = this.path[prePos.x][prePos.y] // 获取上一个点的上一个点
                }
                break;
            }

            for (let i = 0; i < 4; i++) {
                let newX = curPos.x + this.offset[i][0]
                let newY = curPos.y + this.offset[i][1]
                if (this.isArea(newX, newY) && this.maze[newX][newY] === this.road && !this.findPathVisited[newX][newY]) {
                    this.path[newX][newY] = { x: curPos.x, y: curPos.y } // 记录新的点以及该点由谁走过来
                    stack.push({ x: newX, y: newY }) 
                }
            }
        }
        if (!this.hasFindPath) return alert('迷宫无解！')
    }

    // 渲染迷宫指定位置
    findPathReset(x, y, color = '#fff') {
        // 只有不越界的点才做后续处理
        if (this.isArea(x, y)) {
            // 改变dom中对应的节点颜色
            let changeSpan = document.querySelector(`div[data-index='${x}-${y}-${this.class}']`)
            if (x !== this.entryX || y !== this.entryY) {
                changeSpan.style.background = color
            }
        }
    }
}

// 游戏类
class Game {
    constructor() {
        this.mazeSize = 31; // 中等大小的迷宫
        this.maze = new Maze(this.mazeSize, this.mazeSize, 500, 500);
        this.playerPos = { x: this.maze.entryX, y: this.maze.entryY };
        this.steps = 0;
        this.startTime = null;
        this.timerInterval = null;
        this.gameCompleted = false;
        
        this.initializeGame();
        this.setupEventListeners();
    }
    
    initializeGame() {
        this.maze.paintMaze();
        this.updatePlayerPosition();
        this.updateStats();
        this.startTimer();
    }
    
    updatePlayerPosition() {
        // 移除旧的玩家位置
        const oldPlayer = document.querySelector('.player');
        if (oldPlayer) {
            oldPlayer.remove();
        }
        
        // 添加新的玩家位置
        const playerCell = document.querySelector(`div[data-index='${this.playerPos.x}-${this.playerPos.y}-${this.maze.class}']`);
        if (playerCell) {
            const player = document.createElement('div');
            player.className = 'player';
            playerCell.appendChild(player);
        }
    }
    
    movePlayer(dx, dy) {
        if (this.gameCompleted) return;
        
        const newX = this.playerPos.x + dx;
        const newY = this.playerPos.y + dy;
        
        if (this.maze.isArea(newX, newY) && this.maze.maze[newX][newY] === this.maze.road) {
            this.playerPos.x = newX;
            this.playerPos.y = newY;
            this.steps++;
            this.updateStats();
            this.updatePlayerPosition();
            
            // 检查是否到达终点
            if (newX === this.maze.outX && newY === this.maze.outY) {
                this.completeGame();
            }
        }
    }
    
    updateStats() {
        document.getElementById('steps').textContent = this.steps;
        
        if (this.gameCompleted) {
            document.getElementById('status').textContent = '已完成';
        } else {
            document.getElementById('status').textContent = '探索中';
        }
    }
    
    startTimer() {
        this.startTime = new Date();
        
        if (this.timerInterval) {
            clearInterval(this.timerInterval);
        }
        
        this.timerInterval = setInterval(() => {
            if (!this.gameCompleted) {
                const currentTime = new Date();
                const elapsedTime = new Date(currentTime - this.startTime);
                const minutes = elapsedTime.getMinutes().toString().padStart(2, '0');
                const seconds = elapsedTime.getSeconds().toString().padStart(2, '0');
                document.getElementById('timer').textContent = `${minutes}:${seconds}`;
            }
        }, 1000);
    }
    
    completeGame() {
        this.gameCompleted = true;
        clearInterval(this.timerInterval);
        
        const message = document.getElementById('message');
        message.innerHTML = `
            <div>恭喜！你完成了迷宫！</div>
            <div>步数: ${this.steps}</div>
            <div>时间: ${document.getElementById('timer').textContent}</div>
        `;
        message.style.display = 'block';
        
        setTimeout(() => {
            message.style.display = 'none';
        }, 5000);
    }
    
    restartGame() {
        this.playerPos = { x: this.maze.entryX, y: this.maze.entryY };
        this.steps = 0;
        this.gameCompleted = false;
        
        // 重新初始化迷宫
        for (let i = 0; i < this.mazeSize; i++) {
            this.maze.visited[i] = new Array(this.mazeSize).fill(false);
        }
        
        this.maze.initData(this.maze.maze);
        this.maze.initDOM(this.maze.maze);
        this.updatePlayerPosition();
        this.updateStats();
        this.startTimer();
    }
    
    generateNewMaze() {
        this.playerPos = { x: this.maze.entryX, y: this.maze.entryY };
        this.steps = 0;
        this.gameCompleted = false;
        
        // 创建新的迷宫实例
        this.maze = new Maze(this.mazeSize, this.mazeSize, 500, 500);
        this.maze.paintMaze();
        this.updatePlayerPosition();
        this.updateStats();
        this.startTimer();
    }
    
    showSolution() {
        if (this.gameCompleted) return;
        this.maze.findPath(0);
    }
    
    setupEventListeners() {
        document.addEventListener('keydown', (e) => {
            if (e.key === 'w') {
                this.movePlayer(-1, 0);
            } else if (e.key === 's') {
                this.movePlayer(1, 0);
            } else if (e.key === 'a') {
                this.movePlayer(0, -1);
            } else if (e.key === 'd') {
                this.movePlayer(0, 1);
            } else if (e.key === ' ') {
                this.showSolution();
            }
        });
        
        document.getElementById('restart').addEventListener('click', () => {
            this.restartGame();
        });
        
        document.getElementById('generate').addEventListener('click', () => {
            this.generateNewMaze();
        });
        
        document.getElementById('solve').addEventListener('click', () => {
            this.showSolution();
        });
    }
}

// 初始化游戏
document.addEventListener('DOMContentLoaded', () => {
    const game = new Game();
});